...
from RL_brain_cfh import DeepQNetwork
import numpy as np
import random
import time
import pickle
...
def local_observer(sensor,agent):
    ...
    return normfactor*np.concatenate([relu_up_and_down(sensor.dvs_view)])
...
def run_env(eval_mode=False,return_actions=False,return_positions=False,forced_actions=None,forced_positions=None):
    ...
    for episode in range(hp.max_episode):
        ...
        for step_prime in range(hp.steps_per_episode):
            k=hp.decoder_prescale
            if hp.decode_from_dvs:
                if hp.decode_from_central:
                    imim.append(k*sensor.central_dvs_view[...,np.newaxis]+0.)
                else:
                    imim.append(k*sensor.dvs_view[...,np.newaxis]+0.)
            else:
                if hp.decode_from_central:
                    imim.append(k*sensor.central_frame_view[...,np.newaxis]+0.)
                else:
                    imim.append(k*sensor.frame_view[...,np.newaxis]+0.)
            ...
            last_step = step_prime+1 == hp.steps_per_episode
            if last_step:
                observation_ = LAST_ITER_INDICATOR + observation_*0
            # print('debug  reward, last step',reward.reward, last_step)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--decoder_prescale',  default=1.0, type=float, help='scaling of visual input into the decoder')


    parser.add_argument('--decoder_train_en', dest='decoder_train_en', action='store_true')
    parser.add_argument('--no-decoder_train_en', dest='decoder_train_en', action='store_false')
    ...
    parser.add_argument('--replace_target_iter',  default=10, type=int, help='number of learning steps before target network replacd')
    ...
    parser.add_argument('--decode_from_central', dest='decode_from_central', action='store_true')
    parser.add_argument('--no-decode_from_central', dest='decode_from_central', action='store_false')
    ...
    parser.set_defaults(eval_mode=False,
                        ...
                        decoder_train_en=True,
                        decode_from_central=True)
    ...
    observation_size = 36*2#2*(hp.resolution//2)**2+2
    ...
    decoder_input_size = hp.resolution//(2 if hp.decode_from_central else 1)
    if hp.decoder_initial_network is None:
        decoder = rnn_model_102(lr=hp.decoder_learning_rate,ignore_input_B=hp.decoder_ignore_position,dropout=hp.decoder_dropout,rnn_type=hp.decoder_rnn_type,
                                input_size=(decoder_input_size,decoder_input_size, 1))
    else:
