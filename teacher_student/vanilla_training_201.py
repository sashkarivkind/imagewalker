...
# -*- coding: utf-8 -*-
"""
vanilla training of student+decoder
"""
...
parser.add_argument('--full_net_epochs', default=200, type=int, help='num internal training epochs')
...
path = os.getcwd() + '/'
if True:
    teacher = orram_style_nets.parametric_net_befe(dropout1=config['dropout1'],
...
# generator 1
# train_generator_features = Syclopic_dataset_generator(images[:-5000], feature_train_data, **generator_params)
# val_generator_features = Syclopic_dataset_generator(images[-5000:], feature_val_data, validation_mode=True, **generator_params)
# generator 2
train_generator_classifier = Syclopic_dataset_generator(images[:-5000], labels[:-5000], **generator_params)
val_generator_classifier = Syclopic_dataset_generator(images[-5000:], labels[-5000:], validation_mode=True, **generator_params)
...
x = decoder(x)
student_and_decoder = keras.models.Model(inputs=[input0,input1], outputs=x, name='full_net')
...
pre_training_accur = student_and_decoder.evaluate(val_generator_classifier, verbose=2)
...
student_and_decoder.summary()
student_and_decoder_history = student_and_decoder.fit(train_generator_classifier,
                       epochs = parameters['full_net_epochs'] if not TESTMODE else 1,
                       validation_data = val_generator_classifier,
                       verbose = 2,
                       callbacks=[lr_reducer,early_stopper],
            workers=8, use_multiprocessing=True)

# student_and_decoder_history = []


home_folder = save_model_path #+ '{}_saved_models/'.format(this_run_name)
if not os.path.exists(home_folder):
    os.mkdir(home_folder)
# save_model(student_and_decoder, save_model_path, parameters, checkpoint=False)

student_and_decoder.save(home_folder +'decoder_trained_model') #todo - ensure that weights were updated

traject_learning_dataset_update(train_accur,test_accur, student_and_decoder_history, student,parameters, name = 'full_train_103')