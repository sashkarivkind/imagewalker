...
from feature_learning_utils import student1_cnn, student2_cnn, student3_cnn, write_to_file, dataset_update,student32
...
class feature_st(keras.Model):
    ...
    def get_predictions(self, data):
        student_data, teacher_data = data
        student_predictions = self.student(student_data, training=False)
        teacher_target = self.intermediate_layer_model(teacher_data, training = False)
        
        return student_predictions, teacher_target
...
if os.path.exists(path + 'cifar_trained_model2'):
...
train_teacher_data  = trainX[:45000]
test_teacher_data  = trainX[45000:]
#%%
##################### Define Student #########################################
epochs = 1
verbose = 1
...
target = intermediate_layer_model(test_teacher_data[50:150])[:,:,:,feature]

student_net1 = student32()
...
feature_learning.compile(optimizer = keras.optimizers.Adam(lr = 1e-3),
            ...
            feature = feature)
feature_learning.evaluate(test_dataset_x[0], test_teacher_data)
student_history1 = feature_learning.fit(train_dataset_x[0],
                train_teacher_data,
                batch_size = 32,
                epochs = epochs,
                validation_data=(test_dataset_x[0], test_teacher_data),
                verbose = verbose)
...
# student2_prediction = feature_learning2.get_predictions(test_dataset_x[0][50:150]) * 1
...
feature_learning3.compile(optimizer = keras.optimizers.Adam(lr = 1e-3),
           ...
           features_loss_fn = keras.losses.MeanSquaredError(),
           feature = 42)
feature_learning3.evaluate(test_dataset_x[0], test_teacher_data)
...
now = datetime.now()
print(now.strftime("%d/%m/%Y %H:%M:%S"))
student3_prediction, teacher_target = feature_learning3.get_predictions([test_dataset_x[0][50:150],test_teacher_data[50:150]]) * 1
...
teacher_mean, teacher_std = feature_learning3.get_teacher_std(test_teacher_data)
parameters['teacher_mean'] = teacher_mean
parameters['teacher_var'] = teacher_std**2
print('teacher feature mean =', teacher_mean, 'teacher feature var =', teacher_std**2 )
...
with open(prediction_data_path + 'predictions_2_{}_{}_{}'.format(layer_name, feature, run_index), 'wb') as file_pi:
    pickle.dump((target, teacher_target,student3_prediction), file_pi) #student1_prediction,student2_prediction,
now = datetime.now()
print(now.strftime("%d/%m/%Y %H:%M:%S"))
