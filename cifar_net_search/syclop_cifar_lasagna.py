...
def lasagna_model(n_timesteps = 5, hidden_size = 128,input_size = 32, concat = True):
    '''
    ...
    # define LSTM model
    x = keras.layers.LSTM(hidden_size,input_shape=(n_timesteps, None),return_sequences=True)(x)
    x = keras.layers.Flatten()(x)
    x = keras.layers.Dense(10,activation="softmax")(x)
    model = keras.models.Model(inputs=[inputA,inputB],outputs=x, name = 'lasagna_model_{}'.format(concat))
...
rnn_net = lasagna_model(n_timesteps = sample, hidden_size = hidden_size,input_size = 32, concat = True)
...
plt.legend()
plt.title('{} on cifar res = {} hs = {}'.format(rnn_net.name, res, hidden_size))
plt.savefig('{} on Cifar res = {} val accur = {} hs = {}.png'.format(rnn_net.name,res,rnn_history.history['val_sparse_categorical_accuracy'][-1], hidden_size))

with open('/home/labs/ahissarlab/orra/imagewalker/cifar_net_search/{}HistoryDict{}'.format(rnn_net.name, hidden_size), 'wb') as file_pi:
