'''
The follwing code runs a test different interpolations RNN network on the CIFAR dataset 
...
def bad_res101(img,res):
    sh=np.shape(img)
    dwnsmp=cv2.resize(img,res, interpolation = cv2.INTER_NEAREST)
    upsmp = cv2.resize(dwnsmp,sh[:2], interpolation = cv2.INTER_NEAREST)
    return upsmp
...
plt.legend()
plt.title('Interpo Basic RNN on cifar res = {} intorp = {}'.format(res, 'INTER_NEAREST'))
plt.savefig('Interpo Basic RNN on Cifar res = {} intorpo = {}.png'.format(res,'INTER_NEAREST'))

with open('/home/labs/ahissarlab/orra/imagewalker/cifar_net_search/InterpoBasicRnnHistoryDict{}_{}'.format(hidden_size,'INTER_NEAREST'), 'wb') as file_pi:
    pickle.dump(rnn_history.history, file_pi)
    
with open('/home/labs/ahissarlab/orra/imagewalker/cifar_net_search/Interpo{}1imgCnnHistoryDict'.format('INTER_NEAREST'), 'wb') as file_pi:
