...
def cnn_convlstm(n_timesteps = 5, hidden_size = 128,input_size = 32, concat = True):
    ...
    # define LSTM model
    x = keras.layers.ConvLSTM2D(hidden_size,input_shape=(n_timesteps, None),return_sequences=True,recurrent_dropout=rnn_dropout)(x)
...
rnn_net = cnn_convlstm(n_timesteps = sample, hidden_size = hidden_size,input_size = 32, concat = True)
