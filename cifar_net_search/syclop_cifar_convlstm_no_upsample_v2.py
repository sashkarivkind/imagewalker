...
cnn_dropout = 0.4 rnn_dropout = 0.2 , no cnivlstm_dropout samples = 10, h = 256, epochs = 150, convlstm activation = relu - out.276610 (Based on best results from cnn - gru)

cnn_dropout = 0.4 rnn_dropout = 0.2 , WITH cnivlstm_dropout samples = 10, h = 256, epochs = 150, convlstm activation = relu - out.297290 (Based on best results from cnn - gru)
...
def convlstm(n_timesteps = 5, hidden_size = 128,input_size = 32, concat = True):
    ...
    model = keras.models.Model(inputs=[inputA,inputB],outputs=x, name = 'convlstm_cnn_mix_v2_{}'.format(concat))
