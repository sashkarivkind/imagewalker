...
hp.this_run_name = 'syclop_basic_lstm'
def deploy_logs():
...
plt.legend()
plt.title('Basic RNN on cifar res = {}'.format(res))
plt.savefig('Basic RNN on Cifar res = {} val accur = {}'.format(res,rnn_history.history['val_sparse_categorical_accuracy'][-1]))

with open('/BasicRnnHistoryDict', 'wb') as file_pi:
    pickle.dump(rnn_history.history, file_pi)
    
with open('/1imgCnnHistoryDict', 'wb') as file_pi:
