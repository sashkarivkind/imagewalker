...
hp.description = "hello neu, RMS penalty on the first layer" #, 10x slower learning, x10 longer run "
...
hp.logmode = False
hp.initial_network = 'saved_runs/run_syclop_vfb1_neu.py_noname_1585502244_0/tempX_1.nwk'
...
def run_env():
    ...
    for episode in range(hp.max_episode):
        ...
        for step_prime in range(hp.steps_per_episode):
            ...
            reward.update_rewards(sensor = sensor, agent = agent, network=net[1])
...
if __name__ == "__main__":
    ...
    reward = syc.Rewards(reward_types=['central_rms_intensity', 'speed','network'],relative_weights=[1.0,-float(sys.argv[1]),-100])
