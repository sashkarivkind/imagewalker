...
hp.this_run_name = sys.argv[0] + '_noname_' +sys.argv[-1] + str(int(time.time()))
# hp.description = "only 2nd image from videos 1st frame, penalty for speed, soft q learning"
hp.description = "cnn on stills from movies, ONLY ONE IMAGE, targeting rms-dc of 2nd conv layer; fixed observation reset, only last two layers trained, pre-trained  i.c., high beta"
hp.mem_depth = 1
hp.max_episode = 10000
hp.steps_per_episode = 100
hp.steps_between_learnings = 100
hp.steps_before_learning_begins = 100000
hp.fading_mem = 0.5
recorder_file = 'records.pkl'
hp_file = 'hp.pkl'
hp.contrast_range = [1.0,1.1]
hp.logmode = False
hp.dqn_initial_network = 'saved_runs/run_syclop_generic_cnn_vfb_neu.py_noname_1590422112_0/tempX_1ddd.nwk'  # None #'saved_runs/run_syclop_generic_cnn_vfb_neu.py_noname_1589383850_0/tempX_1.nwk'
...
def run_env():
    ...
    for episode in range(hp.max_episode):
        ...
        for step_prime in range(hp.steps_per_episode):
            ...
            net0 =  RL.dqn.eval_incl_layers(local_shape_fun(0.*observation.reshape([1, -1])))
            action, ent = RL.choose_action(observation.reshape([-1]))
            reward.update_rewards(sensor = sensor, agent = agent, network=net[2]-net0[2])
...
if __name__ == "__main__":
    ...
    # images = read_images_from_path('/home/labs/ahissarlab/arivkind/video_datasets/stills_from_videos/some100img_from20bn/*',max_image=1)
    images = read_images_from_path('/home/bnapp/arivkindNet/video_datasets/stills_from_videos/some100img_from20bn/*',max_image=1)
    ...
    reward = syc.Rewards(reward_types=['central_rms_intensity', 'speed', 'network'],relative_weights=[0.0,-float(sys.argv[1]),100.0])
