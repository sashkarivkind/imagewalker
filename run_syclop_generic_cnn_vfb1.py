...
hp.this_run_name = sys.argv[0] + '_noname_' + sys.argv[-1] + '_' + str(int(time.time()))
# hp.description = "only 2nd image from videos 1st frame, penalty for speed, soft q learning"
hp.description = "testing"# "cnn on mnist, 50% inversed, 5000 images"
hp.mem_depth = 1
...
def run_env():
    ...
    for episode in range(hp.max_episode):
        ...
        for step_prime in range(hp.steps_per_episode):
            action,ee = RL.choose_action(observation.reshape([-1]))
...
if __name__ == "__main__":
    ...
    reward = syc.Rewards(reward_types=['central_rms_intensity', 'speed', 'saccade'],relative_weights=[1.0,-float(sys.argv[1]),0])
