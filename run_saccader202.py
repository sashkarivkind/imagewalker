"""updating centralview size to 32 and planning to add cifar based decoder, working with labels as the inner layer"""
...
hp.description = "padding+fishyey + drift network cifar10, on Stanford dataset"
hp.mem_depth = 1
hp.padding=[32,32]
...
def drift_state_for_integrator(drift_net_state, abs_en=False,softmax_en=True,layer=-1):
    if abs_en:
        return np.abs(drift_net_state[layer])
    elif softmax_en:
        return softmax(drift_net_state[layer])
    else:
