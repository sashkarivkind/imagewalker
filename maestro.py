...
class Controller:
    ...
    def __init__(self,ttyStr='/dev/ttyACM0',device=0x0c):
        ...
        self.Maxs = [0] * 24
        self.target = {}
        self.target['minus'] = [896*4] * 24
        self.target['plus'] = [2096*4] * 24
        self.current_target = [1500*4] * 24
    ...
    # Stop the current Maestro Script
    def moveByVelocity(self,chan,velocity):
        if velocity == 0:
            self.setTarget(chan, self.getPosition(chan))
            return
        elif velocity > 0:
            direction = 'plus'
        elif velocity < 0:
        ...
        if not(self.current_target[chan] == self.target[direction][chan]):
            self.current_target[chan] = self.target[direction][chan]
            self.setTarget(chan,self.current_target[chan])
        self.setSpeed(chan,np.abs(velocity))